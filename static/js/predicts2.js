...
function graphPredictions2(data) {
    ...
    var layout = {
        ...
        title: 'Stock Predictions from the Previous 5 Days'
    }
...
(function() {
    ...
    $('#stock-ticker-form').on("submit", function(event) {
        ...
        if (!isPredicting) {
            ...
            console.log(ticker);
            isPredicting = true;
            $.ajax({ method:'get',url:'/predicts2', data:{ticker}, dataType:'json' }).then(function(response){
                graphPredictions2(response);
                isPredicting = false;
