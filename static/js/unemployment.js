        ...
        .html(function(d) {
            if (chosenYAxis === "unemp") {
                if (d.Year.getFullYear() === 2020) {
                    return (`<strong>${new Date(d.Year).getFullYear()}</strong><br> Expected Unemployment Rate: ${d['Unemployment Rate']}%`)
                }
                else {
                    return (`<strong>${new Date(d.Year).getFullYear()}</strong><br> Unemployment Rate: ${d['Unemployment Rate']}%`)
                }
                
            }
            else if (chosenYAxis === "gdp") {
                if (d.Year.getFullYear() === 2020) {
                    return (`<strong>${new Date(d.Year).getFullYear()}</strong><br> Expected GDP: ${d['GDP Growth']}%`)
                }
                else {
                    return (`<strong>${new Date(d.Year).getFullYear()}</strong><br> GDP Growth: ${d['GDP Growth']}%`)
                }
...
function tabulate(data, columns) {
    data = data.map(function(d) {
        var record = {};
        record['Unemployment Rate'] = d['Unemployment Rate'] + "%";
        record['GDP Growth'] = d['GDP Growth'] + "%";
        record.Inflation = d['Inflation'] + "%";
        record.Year = d.Year;
        return record;
    })
...
$(window).on("load", function() {
    ...
    $.ajax({ method:'get',url:'/unemployment/data'}).then(function(data){
        buildGraph(data);
        tabulate(data, ['Year', 'Unemployment Rate', 'GDP Growth', "Inflation"]);
        

    }).catch(function(error) {
        console.log("/unemployment/data", error);
    })
